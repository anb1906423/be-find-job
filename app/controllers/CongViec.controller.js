...
const traVeCongViec = async (req, res) => {
    if (!req?.params?.id)
        return res
            .status(400)
            .json({ message: "id công việc không được để trống" });
    const congViec = await CongViec.findOne({ _id: req.params.id }).exec();
    if (!congViec) {
        return res
            .status(204)
            .json({ message: `Job ID ${req.params.id} not found` });
    }
...
const traVeTatCaCongViecCuaNhaTuyenDung = async (req, res) => {
    try {
        let emailCty = req.query.emailcty;
        ...
        if (!congViec) {
            return res
                .status(204)
                .json({ message: "Không tìm thấy công việc nào!" });
        }
...
const capNhatCongViec = async (req, res, next) => {
    ...
    if (!document) {
        return next(new BadRequestError(404, "Không tìm thấy công việc!"));
    }
...
const xoaCongViec = async (req, res) => {
    ...
    if (req?.body?.isDeleteAll == true) {
        const [error, data] = await handlePromise(CongViec.deleteMany({}));
        if (error) {
            return next(
                new BadRequestError(
                    500,
    ...
    if (!id)
        return res
            .status(400)
            .json({ message: "id bài đăng không được để trống!" });
    const congViec = await CongViec.findOne({ _id: id }).exec();
    if (!congViec) {
        return res.status(400).json({ message: `congViec ID ${id} not found` });
    }
    const result = await congViec.deleteOne({ _id: id });
    res.json(result);
};
...
const onState = async (req, res, next) => {
    ...
    if (!cong_viec_id)
        return res.status(400).send("Trường cong_viec_id không tồn tại");
    ...
    try {
        ...
        res.status(500).json({
            message: "Đã xảy ra lỗi khi thay đổi trạng thái công việc!",
        });
    }
};
